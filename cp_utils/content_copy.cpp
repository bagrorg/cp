                    ...
                    switch (errno) {
                        ...
                        default:
                            throw std::runtime_error(
                                    "Something wrong with file (READ): " + std::string(strerror(errno)));
                    }
                } else if (readed == 0) {
                    throw std::runtime_error(
                            "File trunkated! Current size: " + std::to_string(current_size) + ", expected size: " +
                            std::to_string(fsize));
                }
            ...
            while (current_size < content_size) {
                ...
                if (writed == -1) {
                    switch (errno) {
                        ...
                        default:
                            throw std::runtime_error(
                                    "Something wrong with file (WRITE): " + std::string(strerror(errno)));
                    }
                } else if (writed == 0) {
                    throw std::runtime_error(
                            "File trunkated! Current size: " + std::to_string(current_size) + ", expected size: " +
