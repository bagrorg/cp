...
namespace posix_helpers {
    ...
    void
    linkat(int __fromfd, const std::string &__from, int __tofd, const std::string &__to, int __flags) {
        errno = 0;
    ...
    WrapperException::WrapperException(ERROR_TYPE flag_, const std::string &strerror_input) : flag_(
            flag_) {
        std::string header;
        switch (flag_) {
            case LINKAT_EXDEV:
    ...
    ERROR_TYPE WrapperException::get_error() const {
        return flag_;
    }
}